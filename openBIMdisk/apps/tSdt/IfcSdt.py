...
from apps.tSdt.cityJsonsemantics import *
import os
...
class IfcSdt:
  ...
  def load_cityjson(selfself, cityjson_filename:str, jsonfile:str) -> cityJsonSemantics:
    cityJson = cityJsonSemantics()
    cityJson.loadjson(cityjson_filename)
    return cityJson
  ...
  def remove_minor_dup(self, ifc_file: str) -> IfcSemantics:
    ...
    return ifc0, idDict0, groupObj, self.time_elapsed
  ...
  def compute_diff(self, ifc0: IfcSemantics, ifc1: IfcSemantics) -> dict:
    ...
    ifc0.addIdentical(ifc1)
    ifc1.addIdentical(ifc0)
    preprocessing_period = time.time() - preprocessing_time
    if self.Verbose:
      print("The preprocessing of ifcJson file cost " + str(preprocessing_period) + "s")
    ...
    # record time
    self.time_elapsed = time.time() - starttime
    time_total = time.time() - preprocessing_time
    if self.Verbose:
  ...
  def compute_cityJson(self, cityJson0:str, cityJson1:str) -> cityJsonSemantics:
    t_file_start = time.time()
    if cityJson0[-5:].lower() == '.json':
      if self.Verbose:
        print('>Start handling .cityjson files ...')
        print('Loading ' + cityJson0 + ' ...')
      cityJson_t0 = self.load_cityjson(cityJson0, 'cityJson0.json')
      if self.Verbose:
        print('Loading ' + cityJson1 + ' ...')
      cityJson_t1 = self.load_cityjson(cityJson1, 'cityJson1.json')
    if self.Verbose:
      print('>cityJson files loaded in ' + str(round(time.time() - t_file_start, 3)) + 's.\n')
    starttime = time.time()
    if self.Verbose:
      print('>Start computing diff of two cityJson files ... \n')
    cityJson_t0.load(cityJson_t0.data)
    cityJson_t1.load(cityJson_t1.data)
    if self.Verbose:
      self.log_models(cityJson_t0, cityJson_t1, '1-compared_objects')
    dict0 = cityJson_t0.getDictObj()
    dict1 = cityJson_t1.getDictObj()
    # print(str(dict0))
    for k in dict0:
      if k in dict1 and k != 'features_item':
          s1 = json.dumps(dict0[k], indent=None, sort_keys=True)
          s2 = json.dumps(dict0[k], indent=None, sort_keys=True)
          if s1 == s2:
            dict0[k].clear()
            dict0[k].clear()
      elif k in dict1 and k == 'features_item':
        for key in dict0[k]:
          if key in dict1[k]:
            s1 = json.dumps(dict0[k][key], indent=None, sort_keys=True)
            s2 = json.dumps(dict1[k][key], indent=None, sort_keys=True)

    result = diff(cityJson_t0.getObj(), cityJson_t1.getObj(), syntax='symmetric', marshal=True)
    self.time_elapsed = time.time() - starttime
    self.current_sdt = result
    if self.Verbose:
      with open(self.log_folder + '2-diff-unzipped.json', 'w') as outfile:
