...
import sqlite3
import os
from flask import  render_template, redirect, request, url_for, session, send_from_directory, g, flash, jsonify, stream_with_context, Response
...
def tsdt_function():
    if request.method == 'POST':
        ...
        result_filePath, result_changeData, module_data = my_sdt_route(file_pth_1, file_pth_2, project_openBIM)
...
def my_sdt_route(file1,file2, projName):
    ...
    ifc1, idDict1, groupObj1, computingTime1 = sdt.remove_minor_dup(args.ifc1)
    progress_bar_ratio = 0.33
    ifc2, idDict2, groupObj2, computingTime2 = sdt.remove_minor_dup(args.ifc2)
    progress_bar_ratio = 0.67
    result, time_total = sdt.compute_diff(ifc1,ifc2)
...
def progress():
    ...
    def generate():
        ...
        while ratio < 1:
            ...
            ratio = get_bar_ratio()
            time.sleep(200)
    return Response(generate(), mimetype='text/text-stream')
...
def computeResult(filename):
    if request.method == 'POST':
        ...
        output_diff_folder = os.path.join(output_diff, project_openBIM)
        ...
        output_diff_folder = os.path.join(output_diff, project_openBIM)
        file = send_from_directory(output_diff_folder, filename, as_attachment=True)
        return render_template('home/diff_result.html', result = filename, projName = project_openBIM, blob=file)
...
def restoreResult(filename):
    if request.method == 'POST':
        ...
        output_diff_folder = os.path.join(output_diff, project_openBIM)
        file = send_from_directory(output_diff_folder, filename, as_attachment=True)
        return render_template('home/restore_result.html', result = filename, projName = project_openBIM, blob=file)
...
# The following part defines the tsdt for openGIS page

@blueprint.route('/to_tsdt_gis', methods=['GET', 'POST'])
def to_tsdt_gis():
    if request.method == 'GET':
        return redirect(url_for('home_blueprint.gis_preload'))
    
@blueprint.route('/gis_preload', methods=['GET','POST']) 
def gis_preload():
    logFolders= []
    for dirname in os.listdir(os.path.join(log_folder, "HK")):
        logFolders.append(dirname)
    return render_template('home/tSDT_computing_forGIS.html', logFolders=logFolders)

@blueprint.route('/openGIS', methods = ['POST'])
def openGIS():
    if request.method == 'POST':
        data = request.json
        files = data.get('fileNames', [])
        ...
        file_pth_1 = os.path.join(upload_folder,fileName1)
        file_pth_2 = os.path.join(upload_folder,fileName2)
        result_filePath, result_changeData = compute_gis(file_pth_1, file_pth_2, project_openGIS)
        # upload to blockchain network
        user_name = "org1.example.com"
        user_authority = "Admin"
        channel_name = "gischannel"
        tx_id = "HK" + fileName1.split('.')[0] + '-' + fileName2.split('.')[0]
        activity = 'sdt'
        if 'v' in fileName1 and 'v' in fileName2:
            baseversion = 'V' + fileName1.split('.')[0].split('v')[1]
            targetversion = 'V' + fileName2.split('.')[0].split('v')[1]
        ...
        result_fileName = os.path.basename(filePath)
        trans_time = time.ctime(os.path.getctime(filePath))
        fileType = 'cityJSON_sdt'
        fileSize = str(round(float(os.path.getsize(filePath)),2)) + 'KB'
        args = [tx_id,activity,baseversion,targetversion,filePath,fileType,fileSize]
        database = getattr(g, 'db', None)
        if database is None:
            database = g.db = sqlite3.connect(sqlDB)
        transID = invoke_transaction_gis(user_name, user_authority, channel_name, args)
        result_changeDetail = result_changeData

        # insert the change detail to the database, add a new table in the cached database
        for i in range(0,len(result_changeDetail)):
            item = result_changeDetail[i] # each item is a dictionary
            for key in item:
                component_type = key
                guid_ll = len(item[key]) # list of the identity changed type, e.g., buildings
                for i in range (0,guid_ll):
                    item_data = item[key][i].replace("(","").replace(")","").split(",")
                    graphicID = item_data[0]
                    changeType = item_data[1]
                    file_ver = item_data[2]
                    if file_ver == "baseVer":
                        isBase = 1
                        ...
                        isTarget = 1
                    cursor = database.cursor()
                    cursor.execute("INSERT OR REPLACE INTO gis_info (transID, graphicID, componentType, changeType, isBase, isTarget) values (?,?,?,?,?,?)", (transID, graphicID, component_type, changeType, isBase, isTarget))
                    database.commit()
        # Upload result to the local sqlite3 database
        cursor = database.cursor()
        cursor.execute("INSERT OR REPLACE INTO blockchainTrans (baseVersion, targetVersion,transID,tsdtName,transTime) values (?,?,?,?,?)", (baseversion, targetversion,transID, result_fileName, trans_time))
        database.commit()
        database.close()
    return redirect(url_for('home_blueprint.computeResultGis', filename=result_fileName))

def compute_gis(file1,file2, projName):
    parser = argparse.ArgumentParser(description='Compute tSDT of two cityJSON files.')
    parser.add_argument('-a', '--cityJson1', type=str, help='input cityJSON file path #A')
    parser.add_argument('-b', '--cityJson2', type=str, help='input cityJSON file path #B')
    parser.add_argument('-o', '--output', type=str, help='output json file path')
    parser.add_argument('-v', '--verbose', action='store_true', help='logging verbose details to cityJSON_log_folder')
    args,unknown = parser.parse_known_args()
    args.cityJson1 = file1
    args.cityJson2 = file2
    args.output = output_diff + projName + '/'
    if not os.path.exists(args.output):
        os.makedirs(args.output)
    # generate the result file name accordingly
    if "v" in file1 and "v" in file2:
        subfile1 = file1.split("v")[1].split(".")[0]
        subfile2 = file2.split("v")[1].split(".")[0]
        filename = 'v' + subfile1 + '-v' + subfile2 + '.sdt'
    else:
        filename = 'v' + file1[-9:-5] + '-v' + file2[-9:-5] + '.sdt' 
    IfcSdt.log_folder = LOG_FOLDER_PROJ(file1, file2, projName) + "/"
    args.verbose = 'verbose'
    sdt = IfcSdt()
    sdt.Verbose = args.verbose
    # ifc1 = sdt.remove_minor_dup(args.ifc1)
    # ifc2 = sdt.remove_minor_dup(args.ifc2)
    result = sdt.compute_cityJson(args.cityJson1, args.cityJson2)
    print('SDT: Diff compared in '+ str(round(sdt.time_elapsed,3)) + 's')

    # upload the detailed difference to the database and blockchain 
    changed_item = []
    changed_item_type_list =[]
    changed_item_list = []
    for key in result["features_item"]:
        guid_list = []
        if key != "$insert" and key != "$delete":
            changed_type = key.split('-')[0] # e.g. Buildings
            graphicID = key.split('-')[1]
            guid_list.append("("+ graphicID + ",modify,baseVer-targetVer") 
            # diff_item = result["features_item"][key] # this get the changed item in the features_item list
        elif key == "$insert":
            for key in result["features_item"]["$insert"]:
                changed_type = key.split('-')[0] # e.g. Buildings 
                graphicID = key.split('-')[1]
                guid_list.append("(" + graphicID + ",insert,targetVer)" )
        elif key == "$delete":
            for key in result["features_item"]["$insert"]:
                changed_type = key.split('-')[0] # e.g. Buildings
                graphicID = key.split('-')[1]
                guid_list.append("(" + graphicID + ",delete,baseVer)")       
        
        if changed_type not in changed_item_type_list:
            changed_item_type_list.append(changed_type) # e.g. [Buildings, roads, ...] the changed type list
            changed_item_list.append(guid_list)
            changed_item_by_type = {changed_type : changed_item_list}
        else:
            # update the changed_item_list if the changed type exists
            changed_item_list = changed_item_by_type[changed_type]
            changed_item_list.append(guid_list)
            changed_item_by_type = {changed_type : changed_item_list}
    ...
    filePath = SdtIO.dict_to_base85file(args.output + filename, result)
    return filePath, changed_item

def invoke_transaction_gis(user_name, user_authority, channel_name, args):
    if sys.platform == "win32" and (3, 8, 0) <= sys.version_info < (3, 9, 0):
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    new_loop = asyncio.new_event_loop()
    asyncio.set_event_loop(new_loop)
...
@blueprint.route('/computeResultGIS/<filename>', methods=['GET','POST'])
def computeResultGIS(filename):
    if request.method == 'POST':
        data = request.json
        filename = data.get('resultFileName', [])
        output_diff_folder = os.path.join(output_diff, project_openGIS)
        file = send_from_directory(output_diff_folder, filename, as_attachment=True)
        response = jsonify({'URL': output_diff_folder})
        return render_template('home/diff_result_forGIS.html', result = file)
    elif request.method == 'GET':
        output_diff_folder = os.path.join(output_diff, project_openGIS)
        file = send_from_directory(output_diff_folder, filename, as_attachment=True)
        return render_template('home/diff_result_forGIS.html', result = filename, projName = project_openGIS, blob=file)
